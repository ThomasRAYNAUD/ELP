                ...
                [ -- Section for user input and "Next word" button
                  div [ class "user-input-container" ]
                    [ -- Main title (h1) based on the state of the checkbox
                      h1 [] [ text (if model.checkBoxChecked then Maybe.withDefault "Found me" model.selected else "Find me ...") ]
                    , -- User input field
...
-- Function to render text based on a Maybe String value
renderText : Maybe String -> Html Msg
renderText selected =
    case selected of
        Just word ->
            -- Render the word if it exists
            text word

        Nothing ->
            -- Display a message when no word is available
            text "No word available."

-- Function to render the file load status
renderFileLoadStatus : FileLoadStatus -> Html Msg
renderFileLoadStatus status =
    case status of
        NotLoaded ->
            -- Display a message when the file is not loaded
        ...
        Loaded content ->
            -- Display the content of the loaded file
            pre [] [ text content ]

        LoadError ->
            -- Display an error message when there is an error loading the file
